...
async function getNewModel() {
  let tfModel;
  try {
    trainingLabels = await traininglabel.findOne({ modelType: "layouts"});
...
async function getTestTrainingData(orgId) {
  ...
  trainingData = await trainingdata.find({ orgId: orgId, modelType: "layouts"});
...
module.exports = {
  loadModel: async function (orgId) {
    try {
      const loadedModel = await nnModel.findOne({orgId: orgId, modelType: "layouts"});
      var parsedModel;
      if (loadedModel) {
